...
void ES7210::setup() {
  ...
  // un-mute mic
  ES7210_ERROR_FAILED(this->write_byte(ES7210_ADC34_MUTERANGE_REG14, 0x00));
  ES7210_ERROR_FAILED(this->write_byte(ES7210_ADC12_MUTERANGE_REG15, 0x00));
  this->mic_mute_ = false;
...
bool ES7210::set_mic_mute(bool mic_mute) 
{
	if (this->mic_mute_ == mic_mute)
	{
		return true;
	}
	this->mic_mute_ = mic_mute;

	if (this->setup_complete_) 
	{
		if (mic_mute)
		{
			ES7210_ERROR_CHECK(this->write_byte(ES7210_ADC34_MUTERANGE_REG14, 0x03));
			ES7210_ERROR_CHECK(this->write_byte(ES7210_ADC12_MUTERANGE_REG15, 0x03));
		}
		else
		{
			ES7210_ERROR_CHECK(this->write_byte(ES7210_ADC34_MUTERANGE_REG14, 0x00));
			ES7210_ERROR_CHECK(this->write_byte(ES7210_ADC12_MUTERANGE_REG15, 0x00));
		}
	}
	return true;
}
...
bool ES7210::configure_mic_gain_() {
  ...
  ESP_LOGI(TAG, "configure_mic_gain_() regv=%d", regv);
